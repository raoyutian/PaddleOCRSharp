...
namespace PaddleOCRSharp
{
    ...
    public class PaddleOCREngine : EngineBase
    {
        ...
        internal static extern ulong Initialize(string det_infer, string cls_infer, string rec_infer, string keys, OCRParameter parameter);
        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern ulong Initializejson(string det_infer, string cls_infer, string rec_infer, string keys, string parameterjson);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern IntPtr DetectMat(ulong enginePtr, IntPtr cvmat);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern IntPtr Detect(ulong enginePtr, string imagefile);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern IntPtr DetectByte(ulong enginePtr, byte[] imagebytedata, long size);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern IntPtr DetectBase64(ulong enginePtr, string imagebase64);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern IntPtr DetectByteData(ulong enginePtr, IntPtr imgPtr, int nWidth, int nHeight, int nChannel);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern void FreeEngine(ulong enginePtr);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern bool libModifyParameter(ulong enginePtr, ModifyParameter parameter);

        [DllImport(dllName, CallingConvention = CallingConvention.StdCall, SetLastError = true)]
        internal static extern bool libEnableDetUseRect(ulong enginePtr, bool enable);
        ...
        public PaddleOCREngine(OCRModelConfig config, OCRParameter parameter) : base()
        {
            ...
            if (config == null) config = OCRModelConfig.Default;
            enginePtr = Initialize(config.det_infer, config.cls_infer, config.rec_infer, config.keys, parameter);
            if (enginePtr == 0) throw new Exception("Initialize err：" + GetLastError());
        }
        /// <summary>
        /// PaddleOCR识别引擎对象初始化
        /// </summary>
        ...
        public PaddleOCREngine(OCRModelConfig config, string parameterjson) : base()
        {
            ...
            if (string.IsNullOrEmpty(parameterjson))
            {
                ...
                parameterjson = Path.Combine(Path.Combine(parameterjson, "inference"), "PaddleOCR.config.json");

                if (!File.Exists(parameterjson)) throw new FileNotFoundException(parameterjson);
                parameterjson = File.ReadAllText(parameterjson);
            }
            enginePtr = Initializejson(config.det_infer, config.cls_infer, config.rec_infer, config.keys, parameterjson);
            if (enginePtr == 0) throw new Exception("Initialize err：" + GetLastError());
        }
        ...
        /// 对Mat进行文本识别
        /// </summary>
        /// <param name="p_cvmat">Mat对象指针</param>
        /// <returns>OCR识别结果</returns>
        public OCRResult DetectText(IntPtr p_cvmat)
        {
            ZeroThrow(enginePtr);
            if (p_cvmat == IntPtr.Zero) new ArgumentNullException(nameof(p_cvmat));
            var ptrResult = DetectMat(enginePtr, p_cvmat);
        ...
        {
            ZeroThrow(enginePtr);
            if (string.IsNullOrEmpty(imagefile)) new ArgumentNullException(nameof(imagefile));
            if (!File.Exists(imagefile)) throw new Exception($"文件{imagefile}不存在");
            var ptrResult = Detect(enginePtr, imagefile);
            return ConvertResult(ptrResult);
        }
        ...
        {
            ZeroThrow(enginePtr);
            if (imagebyte == null) throw new ArgumentNullException(nameof(imagebyte));
            var ptrResult = DetectByte(enginePtr, imagebyte, imagebyte.LongLength);
            return ConvertResult(ptrResult);
        }
        ...
        public OCRResult DetectTextBase64(string imagebase64)
        {
            ...
            IntPtr ptrResult = DetectBase64(enginePtr, imagebase64);
            return ConvertResult(ptrResult);
        }
        /// <summary>
        ///文本识别
        ...
        {
            ZeroThrow(enginePtr);
            if (imgPtr == IntPtr.Zero) throw new ArgumentNullException(nameof(imgPtr));
            IntPtr ptrResult = DetectByteData(enginePtr, imgPtr, nWidth, nHeight, nChannel);
            return ConvertResult(ptrResult);
        }
        /// <summary>
        /// 结果解析
        ...
        private OCRResult ConvertResult(IntPtr ptrResult)
        {
            ...
            if (ptrResult == IntPtr.Zero)
            {
                ...
                {
                    throw new Exception("PaddleOCR Err：" + err);
                }
                return result;
            }
            try
  ...
  json = Marshal.PtrToStringAnsi(ptrResult);
        ...
        /// <param name="imagefile">图像文件</param>
        /// <returns>表格识别结果</returns>
        public OCRStructureResult DetectStructure(string imagefile)
        {
        ...
        {
            return libModifyParameter(enginePtr, parameter);
        }
        /// <summary>
        ...
        {
            libEnableDetUseRect(enginePtr, enable);
        }
